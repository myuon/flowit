...
api.post("/gas/validate-deployment", async (c) => {
  const user = c.get("user");
  ...
  if (!googleToken) {
    return c.json({ error: "Google authentication required. Please re-login." }, 401);
  }
  ...
  try {
    ...
    if (!driveResponse.ok) {
      if (driveResponse.status === 401) {
        return c.json({
          valid: false,
          error: "Access token is invalid or expired. Please re-authenticate with Google."
        });
      }
      return c.json({
        valid: false,
    ...
    const driveData = await driveResponse.json() as { files?: Array<{ id: string; name: string }> };
    ...
    for (const file of scriptFiles) {
      ...
      const found = deployments.find((d) => d.deploymentId === body.deploymentId);
      if (found) {
        return c.json({
          valid: true,
          scriptId: file.id,
    ...
    let errorMsg = `Deployment not found: ${body.deploymentId}\n`;
